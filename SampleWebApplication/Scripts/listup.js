...
function init(tableId, models) {
    ...
    $("#SampleModel-save").click(function () {
        if ($("#SampleModel-dialog form").valid()) {
            ...
            if (id < 1) {
                SampleWebApplication.Controllers.HomeController.Add(name_1, note, listup, function (message) { alert(message); });
            }
            else {
                SampleWebApplication.Controllers.HomeController.Edit(id, name_1, note, listup, function (message) { alert(message); });
            }
        }
    });
}
...
//# sourceMappingURL=listup.js.map